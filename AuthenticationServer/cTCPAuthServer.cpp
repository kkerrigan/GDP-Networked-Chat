...
*/
bool cAuthTCPServer::initializeWinsock() {
...
*/
bool cAuthTCPServer::createMasterSocket() {
	...
	this->serverAddressInfo.sin_port = htons(8889);
...
*/
bool cAuthTCPServer::bindSocket() {
...
*	Returns: None
*/
void cAuthTCPServer::startListen() {
		...
		for (int i = 0; i < this->MAX_CLIENTS; i++)
		{
			...
			if (FD_ISSET(this->currentSocket, &this->readfds))
			{
				...
				else if (this->valread == 0)
				{
					...
					cAuthServerMessageHandler::getInstance()->removeUserOnDisconnect(this->currentSocket);
				...
				else
				{
					...
					cAuthServerMessageHandler* messageHandler = cAuthServerMessageHandler::getInstance();
...
*	Returns: None
*/
void cAuthTCPServer::destroySocket() {
	closesocket(this->currentSocket);
}
